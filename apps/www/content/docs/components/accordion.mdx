...
<TabsList className="w-min justify-start rounded-full border-2 bg-popover p-1">
  <TabsTrigger
    className="relative rounded-full bg-transparent px-4 py-0 font-semibold text-primary shadow-none transition-none data-[state=active]:text-background data-[state=active]:shadow-none !h-min"
    value="cli"
  >
    CLI
  </TabsTrigger>
...
```bash
npx @pujan-modha/pm-ui@latest add accordion
```
